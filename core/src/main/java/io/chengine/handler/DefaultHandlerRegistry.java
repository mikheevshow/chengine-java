...
public class DefaultHandlerRegistry implements HandlerRegistry {
	...
	private static final Logger log = LogManager.getLogger(DefaultHandlerRegistry.class);
    ...
    private final Map<Class<?>, HandlerMethod> singleHandleMethodMap = new HashMap<>();
	...
	private DefaultHandlerRegistry() {
	}

    public DefaultHandlerRegistry(Properties chengineProperties) {
    ...
    @Override
    public HandlerMethod getSingleByAnnotationClass(Class<?> clazz) {
        return singleHandleMethodMap.get(clazz);
    }
